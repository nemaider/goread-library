...
<tr><form action="edit.php" method="POST">
    ...
    <td>
        <div class="field">
            <input name="id_book" type="text" readonly
            <?php $id_book = $_GET['id_book']; echo 'value="'.$id_book.'"'; ?> >
        </div>
    </td>
    <td>
        <div class="field">
            <input name="author" type="text"  
            <?php $author = $_GET['author']; echo 'value="'.$author.'"'; ?> >
        </div>
    </td>
    <td>
        <div class="field">
            <input name="title" type="text"  
            <?php $title = $_GET['title']; echo 'value="'.$title.'"'; ?> >
        </div>
    </td>
    <td>
        <div class="field">
            <input name="category" type="text"  
            <?php $category = $_GET['category']; echo 'value="'.$category.'"'; ?> >
        </div>
    </td>
    <td>
        <div class="field">
            <input name="price" type="number"  
            <?php $price = $_GET['price']; echo 'value="'.$price.'"'; ?> >
        </div>
    </td>
    <td>
        <div class="field">
            <input name="amount" type="number"  
            <?php $amount = $_GET['amount']; echo 'value="'.$amount.'"'; ?> >
        </div>
    </td>

    <td>
        <button style="margin-top: 0px; height: 0px;">Modify</button>
    </td>
