...
<div class="body-content">
        ...
        if($result = mysqli_query($conn, $query)){
            ...
            if(isset($_SESSION['return_info'])){
                echo $_SESSION['return_info'];
                unset($_SESSION['return_info']);
            }
        }

        echo '<p class="text">Books that you can return in our library: </p>';
        $query2 = "SELECT * FROM borrows WHERE id_user='$id_user'"; //select all borrowed books by user
                   ...
                   while($row2 = $result2->fetch_assoc()) {
                        ...
                        $row = $result->fetch_assoc();
                            if($row2['id_book'] == $row['id_book']) {
                                ...
                                echo '<tr><td class="db-subinfo">'.$row2['id_borrow'].'</td><td class="db-subinfo">'.$row['author'].'</td><td class="db-subinfo">'.$row['title'].'</td>
                                <td class="db-subinfo">'.$row['category'].'</td><td class="db-subinfo">'.$row['price']*0.2.'</td><td class="db-subinfo">'.$row2['day'].'</td><td>'
                                .'<a href="/goread/library/return.php?borrow='.$row2['id_borrow'].'&price='.$row['price']*0.2.'&username='.$_SESSION['username'].'&book_id='.$row2['id_book'].'">Return</a>
                                </td></tr>';
